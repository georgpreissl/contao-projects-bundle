 ...
 * (gilt für das Backend)
 */
if (Input::get('do') == 'project')
{
	$GLOBALS['TL_DCA']['tl_content']['config']['ptable'] = 'tl_project';
	$GLOBALS['TL_DCA']['tl_content']['config']['onload_callback'][] = array('tl_content_project', 'checkPermission');
	$GLOBALS['TL_DCA']['tl_content']['config']['onload_callback'][] = array('tl_content_project', 'generateFeed');
	$GLOBALS['TL_DCA']['tl_content']['list']['operations']['toggle']['button_callback'] = array('tl_content_project', 'toggleIcon');
}
...
class tl_content_projects extends Backend
{
	...
	public function checkPermission()
	{
		...
		switch (Input::get('act'))
		{
			...
			case 'create':
			case 'select':
				// Check access to the project item
	...
	protected function checkAccessToElement($id, $root, $blnIsPid=false)
	{
		if ($blnIsPid)
		{
			$objArchive = $this->Database->prepare("SELECT a.id, n.id AS nid FROM tl_project n, tl_projects_archive a WHERE n.id=? AND n.pid=a.id")
										 ->limit(1)
										 ->execute($id);
		}
		else
		{
			$objArchive = $this->Database->prepare("SELECT a.id, n.id AS nid FROM tl_content c, tl_project n, tl_projects_archive a WHERE c.id=? AND c.pid=n.id AND n.pid=a.id")
										 ->limit(1)
										 ->execute($id);
		}
		...
		{
			$this->log('Invalid project content element ID ' . $id, __METHOD__, TL_ERROR);

			return false;
		}

		// The project archive is not mounted
		if (!in_array($objArchive->id, $root))
		{
			$this->log('Not enough permissions to modify article ID ' . $objArchive->nid . ' in project archive ID ' . $objArchive->id, __METHOD__, TL_ERROR);
