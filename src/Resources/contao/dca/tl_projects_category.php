...
/**
 * Load tl_projects_archive language file
 */
 ...
 */
$GLOBALS['TL_DCA']['tl_projects_category'] = array
(
    ...
    (
        ...
        (
            array('tl_projects_category', 'checkPermission')
        ),
        'sql' => array
        (
            'keys' => array
    ...
    (
        ...
        (
            ...
            'paste_button_callback'   => array('tl_projects_category', 'pasteCategory'),
            'panelLayout'             => 'search'
        ),
        'label' => array
        (
            ...
            'label_callback'          => array('tl_projects_category', 'generateLabel')
        ),
        ...
        (
            ...
            (
                ...
                'button_callback'     => array('tl_projects_category', 'toggleIcon')
            )
        )
    ),
    ...
    (
        ...
        (
            ...
            'save_callback' => array
            (
                array('tl_projects_category', 'generateAlias')
            ),
 ...
 */
/*
if (\GeorgPreissl\Projects\ModuleProjectsCategories::checkMultilingual()) {

    // Config
    $GLOBALS['TL_DCA']['tl_projects_category']['config']['dataContainer'] = 'Multilingual';
    $GLOBALS['TL_DCA']['tl_projects_category']['config']['language'] = \GeorgPreissl\Projects\ModuleProjectsCategories::getAvailableLanguages();
    $GLOBALS['TL_DCA']['tl_projects_category']['config']['langColumn'] = 'language';
    $GLOBALS['TL_DCA']['tl_projects_category']['config']['langPid'] = 'lid';
    $GLOBALS['TL_DCA']['tl_projects_category']['config']['fallbackLang'] = \GeorgPreissl\Projects\ModuleProjectsCategories::getFallbackLanguage();
    $GLOBALS['TL_DCA']['tl_projects_category']['config']['sql']['keys']['language'] = 'index';
    $GLOBALS['TL_DCA']['tl_projects_category']['config']['sql']['keys']['lid'] = 'index';

    // Fields
    $GLOBALS['TL_DCA']['tl_projects_category']['fields']['language']['sql'] = "varchar(2) NOT NULL default ''";
    $GLOBALS['TL_DCA']['tl_projects_category']['fields']['lid']['sql'] = "int(10) unsigned NOT NULL default '0'";
...
class tl_projects_category extends Backend
{
    ...
    public function checkPermission()
    {
        ...
        if (!$this->User->isAdmin && !$this->User->projectsscategories) {
    ...
    public function generateAlias($varValue, DataContainer $dc)
    {
        ...
        $objAlias = $this->Database->prepare("SELECT id FROM tl_projects_category WHERE alias=?")
                                   ->execute($varValue);
    ...
    {
        $objVersions = new Versions('tl_projects_category', $intId);
        $objVersions->initialize();

        // Trigger the save_callback
        if (is_array($GLOBALS['TL_DCA']['tl_projects_category']['fields']['published']['save_callback'])) {
            foreach ($GLOBALS['TL_DCA']['tl_projects_category']['fields']['published']['save_callback'] as $callback) {
        ...
        // Update the database
        $this->Database->prepare("UPDATE tl_projects_category SET tstamp=". time() .", published='" . ($blnVisible ? 1 : '') . "' WHERE id=?")
                       ->execute($intId);

        $objVersions->create();
        $this->log('A new version of record "tl_projects_category.id='.$intId.'" has been created'.$this->getParentEntries('tl_projects_category', $intId), __METHOD__, TL_GENERAL);
    }
}
