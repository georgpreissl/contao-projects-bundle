...
namespace GeorgPreissl\Projects\Widget;
...
class ProjectCategoriesPickerWidget extends Widget
{
    /**
     * Submit user input.
     *
    ...
    public function generate()
    {
        ...
        // Can be an array
        if (!empty($this->varValue) && null !== ($categories = ProjectsCategoryModel::findMultipleByIds((array) $this->varValue))) {
  ...
  <div class="selector_container">'.(($this->sorting && count($values) > 1) ? '
    ...
    <ul id="sort_'.$this->strId.'" class="'.($this->sorting ? 'sortable' : '').'">';
        ...
        if (!$pickerBuilder->supportsContext('projectsCategories')) {
            $return .= '
	<p><button class="tl_submit" disabled>'.$GLOBALS['TL_LANG']['MSC']['changeSelection'].'</button></p>';
            ...
            $return .= '
	<p><a href="'.ampersand($pickerBuilder->getUrl('projectsCategories', $extras)).'" class="tl_submit" id="pt_'.$this->strName.'">'.$GLOBALS['TL_LANG']['MSC']['changeSelection'].'</a></p>
	<script>
	  $("pt_'.$this->strName.'").addEvent("click", function(e) {
		e.preventDefault();
		Backend.openModalSelector({
		  "id": "tl_listing",
		  "title": "'.StringUtil::specialchars(str_replace("'", "\\'", ($GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strField]['label'][0] ?? null) )).'",
		  ...
		  "callback": function(table, value) {
			...
			}).post({"action":"reloadProjectCategoriesWidget", "name":"'.$this->strId.'", "value":value.join("\t"), "REQUEST_TOKEN":"'.REQUEST_TOKEN.'"});
		  }
		});
	  });
	</script>'.($this->sorting ? '
    ...
    protected function checkValue($input)
    {
        ...
        if (count(array_diff($ids, array_merge($this->rootNodes, Database::getInstance()->getChildRecords($this->rootNodes, 'tl_projects_category')))) > 0) {
