...
class ProjectsCategoryListener implements FrameworkAwareInterface
{
    ...
    /**
     * ProjectsCategoryListener constructor.
     *
     * @param Connection        $db
    ...
    public function onLoadCallback(DataContainer $dc)
    {
        if (!$this->permissionChecker->canUserManageCategories() && !$this->permissionChecker->canUserAssignCategories()) {
            throw new AccessDeniedException('User has no permissions to manage project categories');
        }
        ...
        // Get the project categories root set previously in session (see #137)
        if (isset($_SESSION['PROJECTS_CATEGORIES_ROOT']) && $input->get('picker')) {
            $GLOBALS['TL_DCA'][$dc->table]['list']['sorting']['root'] = $_SESSION['PROJECTS_CATEGORIES_ROOT'];
            unset($_SESSION['PROJECTS_CATEGORIES_ROOT']);
        } else {
        ...
        if (null !== ($roots = $this->permissionChecker->getUserAllowedRoots())) {
            ...
            switch ($action = $input->get('act')) {
                case 'edit':
                    ...
                    if (!$this->permissionChecker->isUserAllowedProjectCategory($categoryId)) {
                ...
                case 'show':
                    ...
                    if (!$this->permissionChecker->isUserAllowedProjectCategory($categoryId)) {
                        throw new AccessDeniedException(\sprintf('Not enough permissions to %s project category ID %s.', $action, $categoryId));
                    }
                    break;
                case 'editAll':
                case 'deleteAll':
    ...
    public function onGenerateAlias($value, DataContainer $dc)
    {
        ...
        if (!$value) {
            ...
            if (null !== $this->slug) {
                ...
                if (!empty($validChars = Config::get('projects_categorySlugSetting'))) {
